...
export const useCheckId = () => {
  ...
  const checkId = async (userid, zoneid, product, item) => {
    ...
    try {
      ...
      // Handle non-OK HTTP responses (400, 500, 502, etc.)
      if (!response.ok) {
        setVerified(false);
      ...
      if (data.message === "success") {
        ...
        // Smile.one: "USER ID não existe" — translate to user-friendly English
        setVerified(false);
        setMessage("User ID or Server does not exist. Please check your details.");
      } else {
        setVerified(false);
